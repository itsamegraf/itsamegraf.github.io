...
export async function createTrack(ground, screen) {
    ...
    function getBuiltTrackWidth() {
        const screenCount = trackPlan.length-1;
        const lastScreenRow = trackPlan[screenCount]; // row
        const lastScreenCol = lastScreenRow[lastScreenRow.length-1] // column
        return (screenCount - 1 + lastScreenCol[0]) * screen.width;
    }
