...
(async () =>
{
    ...
    const countRow=1, countCol=1, videoTileDimensions = {
    ...
    for (i=0;i<countRow;i++) {
        for (j=0;j<countRow;j++) {
            // Compute dimensions
            const currTileDim = Object.assign({}, 
                videoTileDimensions, 
                { x: (i * videoTileDimensions.w),
                  y: (j * videoTileDimensions.h)
                });

            const frame = new Rectangle(currTileDim.x, currTileDim.y, currTileDim.w, currTileDim.h);
            let currTexture = new Texture({source: baseVideoTexture, frame: frame});
            let videoSprite = new Sprite(currTexture);
            videoSprite.position.x = currTileDim.x;
            videoSprite.position.y = currTileDim.y;
