...
(async () =>
{
    ...
    async function scene1(delta) {
        ...
        const moveCart = (delta) => {
            // TODO: utilise delta - delta * (1000 / 60)
            count += 1;
            world.rc.cart.attachToTrack(Math.floor(count / 20) % 1000);
            world.rc.addDebugLines();
        }
        // app.ticker.add(moveCart);
        world.rc.cart.attachToTrack(10);
        // world.rc.cart.iterCartPoints();
        world.rc.addDebugLines();
        
        // const cartPoints = Array.from(world.rc.cart.iterCartPoints())
        // const cartLine = world.rc.renderPoints(cartPoints);
        // world.rc.cart.container.addChild(cartLine);
    }
